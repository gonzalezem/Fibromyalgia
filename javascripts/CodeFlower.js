...
CodeFlower.prototype.update = function(json) {
  ...
  this.force
    .gravity(Math.atan(total / 1) / Math.PI * 0.6) //choose gravity settings
    .nodes(nodes)
    .links(links)
    .start();
  ...
  this.node.enter().append('svg:circle')
    ...
    .attr("r", function(d) { return d.children ? 6 : Math.pow(d.size, 1/5) *1.5  || 1; })  // the first number gives the diameters of the nodes when there is no size. The second gives give the size of the last children.
